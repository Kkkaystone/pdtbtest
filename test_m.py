...
PATH=new_model="/scratch/user/xishi/pdtb/coding/Mistral-7B-Instruct-v0.2-7b-pdtb2.0-epoch3-p4-fix20240319161911"
...
def eval(model,prompts,test_type):
    ...
    with torch.no_grad():
        ...
        with open('../output/'+'m_output_'+test_type+datetime.now().strftime("%d%H%M%S")+".json", 'w', encoding='utf-8') as file:
...
# with open('../output/m_output_zeroshot19111255.json', 'r', encoding='utf-8') as file:
#     out_list_zeroshot = json.load(file)

# with open('../output/m_output_fewshot19112756.json', 'r', encoding='utf-8') as file:
#     out_list_fewshot= json.load(file)
# with open('../output/m_output_sft19105739.json', 'r', encoding='utf-8') as file:
#     out_list_sft= json.load(file)


label_predict_zeroshot=label_mapping(out_list_zeroshot)
label_predict_fewshot=label_mapping(out_list_fewshot,is_fewshot=True)
